...
7. No file or directory name calls `selectHunk`, updates FileTree highlighting, calculates counters, expands, collapses, or fetches. FileTree Navigation only scrolls and must not invoke any selection calculation afterward.
...
The Next/Previous off-screen-selected-target rule does not apply to a direct file command. FileTree Navigation leaves selection unchanged.
